...
export async function updateUI(resultEl, getResult, streaming) {
  resultEl.className = "loading body-model";
  let text = "";
  try {
    ...
    if (streaming) {
      ...
      for await (const chunk of result.stream) {
        ...
        text += chunkText;
        resultEl.textContent = marked.parse(text)
    ...
    resultEl.className = " body-model"; // Remove .loading class
  } catch (err) {
    text += "\n\n> " + err;
    resultEl.className = "error";
  }
  resultEl.textContent = text;  //`<div class="body">${text}</div>`; //marked.parse(text);
  // scrollToDocumentBottom();
}
